...
class GatherPictureLIRMediaObjects extends Task implements _task {

    const TASK_CLASS = "GatherPictureLIRMediaObjects";
    ...
    /**
     * GatherPictureLIRMediaObjects constructor.
     * @param $data
     */
    ...
    public function documentation() {
        return "<h2>Gather LIR Picture Media Objects</h2>"
            . "<h4>Description:</h4>"
            . "<div>This task creates LIR Picture Media Objects from Build Plan.</div>"
    ...
    public function run() {
        ...
        $outputParams = ['created_picture_lir_media_object_ids' => implode(",", $this->createdMOs),
            'found_picture_lir_media_object_ids' => implode(",", $this->foundMOs),
        ];

        if ($this->invalidRows) {
            $outputParams["found_picture_lir_errors"] = implode(",", $this->invalidRows);
    ...
    private function validateTaskParams() {
        ...
        if (count($titleInfoMOs) > 1) {
            throw new Exception("More than one Title Info MO found");
        }
        ...
        $mediaClass = MediaClass::retrieveByID(51) ;
    ...
    private function processCSV() {
        ...
        while (($data = fgetcsv($handle, 0, ",")) !== FALSE) {
            if ($headers) {
                ...
                $pattern = '/Inserts/i';
                if (preg_match($pattern,$moData['picture']) || ($moData["language_ntt"] && $moData["dim"] == "3D")){
    ...
    private function createMediaObject($moData) {
        ...
        $mediaObject = new MediaObject(MEDIA_CLASS_ID_DC_PICTURE_LIR);
        $mediaObject->setName($moName."-inserts");
        ...
        $mediaObject->setAttributesByName(["DC_Format" => $standard, "Dimension" => $moData["dim"]]);

        $mediaClass = MediaClass::retrieveByID(MEDIA_CLASS_ID_DC_PICTURE_LIR) ;
        ...
        if ($this->params[self::SOURCE_TYPE] == 'DCDM') {
            $path = $this->basePath."/".$moData["dim"]."/".$getSubfolderpathVar1."/".$standard."/".LanguageTable::getLanguageDisneyCodeByID($moData["languageID"])."/";
        } else {
            $path = $this->basePath."/".$moData["dim"]."/".$getSubfolderpathVar2."/".$standard."/".LanguageTable::getLanguageDisneyCodeByID($moData["languageID"])."/";
        }
    ...
    private function validateCSV($data) {
        $columns = [];
        $dataCols = ["name" => "DCP COMPOSITION NAME", "territory" => '^TERRITORY \(DDSS\)$', "picture" => '^PICTURE$', "dim" => '^DIM.$', "standard" => '^STANDARD$', "language" => '^AUDIO LANGUAGE$',"language_ntt" => '^NTT \/ FORCED NARRATIVE LANGUAGE$'];
