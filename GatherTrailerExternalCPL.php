...
require_once (PATH_TO_INC . '/entities/RelatedContent.php');

class GatherTrailerExternalCPL extends Task implements _task {

    const TASK_CLASS = "GatherTrailerExternalCPL";
    ...
    public function documentation() {
        return "<h2>Gather Trailer External CPL Media Objects</h2>"
            . "<h4>Description:</h4>"
            . "<div>This task creates Trailer External CPL Media Objects</div>"
    ...
    public function run() {
        ...
        $outputParams = ['created_trailer_external_cpl_media_object_ids' => implode(",", $this->createdMOs),
            'found_trailer_external_cpl_media_object_ids' => implode(",", $this->foundMOs),
        ];
    ...
    private function createMediaObject($moData) {
        ...
        $moName = $this->titleABBR."_".$moData["language"]."_Trailer_".$moData["standard"];
        $moName = preg_replace('/[^A-Za-z0-9_]/', '', $moName);

        $language = "";
        if (preg_match("/_([A-Za-z\-]+)_([A-Za-z\-]+)_(51|71)/i", $moName, $matches) ) {
            $language = $matches[1];
        }
        $languageArr = explode('-',$language);
        $subtitleLanguage = implode('-',array_splice($languageArr,1));
        ...
        $relatedContentID = RelatedContent::getMaxIdByTitleCategory($this->params[self::TITLE_ID],17);
        $mediaObject->setRelatedContentID($relatedContentID);

        $attrs = ["Audio_Config" => "51", // $moData["audio_config"],
            "DC_Format" => $moData["standard"],
            "Dimension" => $moData["dim"],
            "Subtitle_Language" => $subtitleLanguage, // $moData["subtitle_language"],
            "Reels_Count" => 1,
            "Movie_Title" => $this->titleABBR."TRAILER",
            "Audio_Language" => $moData["audio_language"],
            "Territory" => $this->getTerritory($moData["name"]),
            "Resolution" => $this->getMOResolution($moData["name"]),
            "Content_Kind" => "trailer",
            "Audio-HI" => "false",
            "Audio-VI" => "false",
            "Dbox" => "false",
            "Atmos" => "false"
        ...
        $result = MOUtils::checkMediaObjectUniqueness(0, $mediaObject->getMediaClassID(), $mediaObject->getTitleID(), $mediaObject->getTitleVersionID(), $mediaObject->getLanguageID(), $mediaObject->getSubclass(), $relatedContentID);
        if ($result === true) {
